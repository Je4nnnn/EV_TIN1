...
Proyecto full stack de gestion hotelera con backend Spring Boot, frontend React/Vite, autenticacion con Keycloak, PostgreSQL, Docker y pipeline Jenkins.
...
El compose principal de la raiz levanta un stack autosuficiente con `postgres`, `keycloak`, `backend` y `frontend`.

La app queda pensada para demo con autenticacion real activa:

- backend local con seguridad habilitada
- frontend local y Docker con Keycloak habilitado
- vistas administrativas visibles solo para usuarios con `hotelrm_admin`
- Swagger/OpenAPI disponible en `http://localhost:8091/swagger-ui/index.html`

El backend tambien carga datos demo minimos al levantar `local` o `docker`, para que la app siempre tenga reservas, usuarios y paquetes visibles en la demo.
...
- `CORS_ALLOWED_ORIGINS=http://localhost:5173,http://localhost:3000,http://localhost:3001,http://localhost:4173`
...
- `APP_SECURITY_ENABLED_DOCKER=true`
- `VITE_KEYCLOAK_ENABLED_DOCKER=true`
...
Si vas a mostrar autenticacion al profesor, asegúrate antes de que Keycloak local ya tenga el realm `hotelrm` importado y que tu usuario tenga el rol `hotelrm_admin`.
...
El compose principal esta en la raiz del repo y levanta todo lo necesario para la demo:

```bash
docker compose up --build -d
```

Si quieres reiniciar con base limpia y volver a cargar los datos demo:
...
- Frontend en `http://localhost:3001/home`
- Backend en `http://localhost:8091`
- Swagger en `http://localhost:8091/swagger-ui/index.html`
